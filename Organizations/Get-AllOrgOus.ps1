Function Get-OrgOUList{
[Alias("Get-IAMOrgOUList")]
<#
    .SYNOPSIS
    ...
    .NOTES
        ...
        9/19/2019 - Added root ou to output
...
function Get-OrgChildOUs {
    ...
    foreach ($childouid in $childouids.Id) {
        ...
        $Script:AllOUs += $obj
        Start-Sleep -Seconds .6 # For some reason, AWS cmdlet Get-OrgOrganizationalUnit fails if lots of IDs are passed via pipeline, so this foreach loop slows it down
...
function Get-AllOrgOus {
    ...
    Process {
        ...
        $objarray = @()
        $r = get-orgroot
        $RootOUId = $r.Id # Get the four-character root OU ID
            $obj = new-object psobject
            
                $obj |Add-member NoteProperty Parent "TOP"
                $obj |Add-member NoteProperty OUName $r.name
                $obj |Add-member NoteProperty Id $RootOUId
                $obj |Add-member NoteProperty Arn $r.Arn
                $objarray += $obj
    ...
    End {
        ...
        foreach ($id in $Script:AllOUs.id) {
            $obj = new-object psobject
                ...
                $obj |Add-member NoteProperty Id $($Script:AllOUs.Id[($Script:AllOUs.id).IndexOf($id)]) 
...
Function Get-OrgAccountLocation{
    ...
    $orglist |%{
        $o = $_
        $ChildAccounts = Get-ORGChild -ParentId $_.OUID -ChildType Account
        $childaccounts |%{
            $acct = get-orgaccount -accountid $_.id
            
            $obj = new-object psobject
            #$obj |Add-member NoteProperty OUParent $o.Parent
